...
class TestCleanCode(unittest.TestCase):
    ...
    def test_clean_code_with_sh_prefix(self):
        """测试去除 sh 前缀"""
        self.assertEqual(clean_code("sh600519"), "600519")

    def test_clean_code_with_sz_prefix(self):
        """测试去除 sz 前缀"""
        self.assertEqual(clean_code("sz000559"), "000559")
    ...
    def test_clean_code_without_prefix(self):
        """测试无前缀的情况"""
        self.assertEqual(clean_code("600519"), "600519")

    def test_clean_code_with_hk_prefix(self):
        """测试去除 hk 前缀"""
        self.assertEqual(clean_code("hk00700"), "00700")


class TestGetRecentTradeDates(unittest.TestCase):
    """get_recent_trade_dates 函数测试"""
    ...
    def test_get_recent_trade_dates_single(self):
        """测试获取单个交易日日期"""
        dates = get_recent_trade_dates(1)
...
class TestCalculateDarkTradeStats(unittest.TestCase):
    ...
    @patch("stock_monitor.services.dark_trade.calculator.fetch_all_dark_trade")
    @patch("stock_monitor.services.dark_trade.calculator.build_net_flow_index")
...
class TestPushDarkTradeStats(unittest.TestCase):
    ...
    @patch("stock_monitor.services.dark_trade.pusher.calculate_dark_trade_stats")

...
class TestStockTableModel(unittest.TestCase):
    ...
    def test_row_column_count(self):
        ...
        self.assertEqual(self.model.columnCount(), 4) # Should include seal column due to 4th item
    ...
    def test_data_display(self):
        ...
        self.assertEqual(self.model.data(idx_pingan, QtCore.Qt.ItemDataRole.DisplayRole), " 平安银行")
        
        idx_tencent = self.model.index(2, 0)
        self.assertEqual(self.model.data(idx_tencent, QtCore.Qt.ItemDataRole.DisplayRole), " 腾讯控股")
        
        # Test Price
        idx_price = self.model.index(0, 1)
        self.assertEqual(self.model.data(idx_price, QtCore.Qt.ItemDataRole.DisplayRole), "10.50")
        
        # Test Change Formatting
        idx_change_raw = self.model.index(0, 2)
        self.assertEqual(self.model.data(idx_change_raw, QtCore.Qt.ItemDataRole.DisplayRole), "1.23%")
        
        idx_change_pct = self.model.index(1, 2)
        self.assertEqual(self.model.data(idx_change_pct, QtCore.Qt.ItemDataRole.DisplayRole), "-2.5% ")
    ...
    def test_alignment(self):
        ...
        align_name = self.model.data(self.model.index(0, 0), QtCore.Qt.ItemDataRole.TextAlignmentRole)
        self.assertTrue(align_name & QtCore.Qt.AlignmentFlag.AlignLeft)
        
        # Price should be right aligned
        align_price = self.model.data(self.model.index(0, 1), QtCore.Qt.ItemDataRole.TextAlignmentRole)

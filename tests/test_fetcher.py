...
class TestStockFetcher(unittest.TestCase):
    ...
    def test_fetch_a_stocks_limit(self, mock_use):
        ...
        with patch.object(self.fetcher, '_fetch_hk_stocks', return_value=[]), \
    ...
    def test_fetch_hk_stocks_parsing(self, mock_get):
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df.to_excel(writer, index=False, startrow=1) # header=1 in read_excel means row 1 is header (0-indexed? usually row 0 is title, row 1 header)
        ...
        output = io.BytesIO()
        df_full.to_excel(output, index=False, header=False) # Write without header, raw data

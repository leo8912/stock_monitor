...
class TestLRUCacheMechanism(unittest.TestCase):
    ...
    def test_same_stock_data_request_should_hit_cache(self):
        ...
        # 检查缓存信息
        cache_info_before = self.stock_manager._process_single_stock_data_cached.cache_info()
        ...
        # 验证缓存命中
        cache_info_after = self.stock_manager._process_single_stock_data_cached.cache_info()
    ...
    def test_different_stock_data_request_should_not_hit_cache(self):
        ...
        # 分别调用
        result1 = self.stock_manager._process_single_stock_data_cached(code1, info_json1)
        result2 = self.stock_manager._process_single_stock_data_cached(code2, info_json2)
    ...
    def test_edge_cases_empty_data(self):
        ...
        # 无效JSON
        result = self.stock_manager._process_single_stock_data_cached("invalid", "invalid json")
        self.assertIsInstance(result, tuple)
        self.assertEqual(len(result), 6)  # 应该返回6个元素的元组

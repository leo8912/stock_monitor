"""自选股 / 全量 A 股 Excel 导出（包内实现）。

历史实现位于 ``scripts/reporting/export_stocks_to_excel.py``；打包后
``scripts`` 不在导入路径，运行时调用方一律从本模块导入。
"""
...
def export_to_excel(
    ...
    for sheet_name in wb.sheetnames:
        ...
        for col_idx, col in enumerate(ws.columns, 1):
            ...
            max_len = sum(2 if "一" <= char <= "鿿" else 1 for char in header_value)
            is_numeric = False
            if any(
                k in header_value
                for k in [

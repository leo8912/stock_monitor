...
class StockDataValidator:
    ...
    @staticmethod
    def handle_special_cases(info: Dict[str, Any], pure_code: str, code: str, should_copy: bool = False) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_stock_info(data: Dict[str, Any], code: str) -> Optional[Dict[str, Any]]:
        ...
        if info and isinstance(data, dict):
            # 提取纯数字代码
            pure_code = code[2:] if code.startswith(('sh', 'sz')) else code
            info = StockDataValidator.handle_special_cases(info, pure_code, code, should_copy=True)

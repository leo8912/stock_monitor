    ...
    def get_realtime_price(self, code: str) -> dict:
        ...
        try:
            ...
            return {
                ...
                "change_pct": (float(parts[3]) - float(parts[2]))
                / float(parts[2])
                * 100
                if float(parts[2]) > 0
                else 0,
...
class BaostockAPI:
    ...
    def get_kline_data(
        self, code: str, period: str = "daily", days: int = 60
    ) -> pd.DataFrame:
        """获取K线数据"""
        if not self.bs:
            print("Baostock未初始化")
...
def generate_detailed_report(df, patterns, realtime_data, stock_code):
    ...
    if realtime_data:
        ...
        change = (
            latest["change_pct"] / 100 * latest["preclose"] if latest["preclose"] else 0
        )
        stock_name = ""
        ...
        if current_price > latest["MA5"]:
            ma_signals.append(
                (
                    "MA5支撑",
                    0.5,
                    f"股价({current_price:.2f})>MA5({latest['MA5']:.2f})，短期支撑有效",
                )
            )
        else:
            ma_signals.append(
                (
                    "MA5压力",
                    -0.5,
                    f"股价({current_price:.2f})<MA5({latest['MA5']:.2f})，短期承压",
                )
            )

    if "MA20" in df.columns and not pd.isna(latest["MA20"]):
        if current_price > latest["MA20"]:
            ma_signals.append(("MA20支撑", 0.5, "股价在中期均线上方，中期趋势向好"))
        else:
            ma_signals.append(("MA20压力", -0.5, "股价在中期均线下方，中期趋势走弱"))

    if "MA5" in df.columns and "MA10" in df.columns and "MA20" in df.columns:
        if latest["MA5"] > latest["MA10"] > latest["MA20"]:
            signals.append(
                ("均线多头排列", 1.5, "MA5>MA10>MA20，上升趋势确立，均线呈多头排列")
            )
            signal_weights.append(1.5)
        elif latest["MA5"] < latest["MA10"] < latest["MA20"]:
            signals.append(
                ("均线空头排列", -1.5, "MA5<MA10<MA20，下降趋势确立，均线呈空头排列")
            )
            signal_weights.append(-1.5)
        ...
        if rsi < 20:
            signals.append(
                (
                    "RSI极度超卖",
                    1.5,
                    f"RSI={rsi:.1f}<20，极度超卖，强烈反弹信号，趋势{rsi_trend}",
                )
            )
            signal_weights.append(1.5)
        elif rsi < 30:
            signals.append(
                (
                    "RSI超卖",
                    1,
                    f"RSI={rsi:.1f}<30，超卖区域，存在反弹机会，趋势{rsi_trend}",
                )
            )
            ...
            signals.append(
                (
                    "RSI极度超买",
                    -1.5,
                    f"RSI={rsi:.1f}>80，极度超买，强烈回调风险，趋势{rsi_trend}",
                )
            )
            signal_weights.append(-1.5)
        elif rsi > 70:
            signals.append(
                (
                    "RSI超买",
                    -1,
                    f"RSI={rsi:.1f}>70，超买区域，注意回调风险，趋势{rsi_trend}",
                )
            )
            signal_weights.append(-1)
        elif 50 < rsi <= 70:
            signals.append(
                (
                    "RSI强势",
                    0.5,
                    f"RSI={rsi:.1f}，健康强势区，买方占优，趋势{rsi_trend}",
                )
            )
            signal_weights.append(0.5)
        elif 30 <= rsi < 50:
            signals.append(
                ("RSI弱势", -0.5, f"RSI={rsi:.1f}，偏弱区域，卖方占优，趋势{rsi_trend}")
            )
            signal_weights.append(-0.5)
            ...
            if (
                ...
                signals.append(
                    (
                        "MACD金叉",
                        1.5,
                        f"DIF({macd_val:.3f})刚刚上穿DEA({signal_val:.3f})，金叉确立，买入信号强烈",
                    )
                )
                signal_weights.append(1.5)
            else:
                signals.append(
                    (
                        "MACD金叉延续",
                        1,
                        f"DIF({macd_val:.3f})>DEA({signal_val:.3f})，金叉延续，动能{'增强' if hist_val > hist_prev else '减弱'}",
                    )
                )
                signal_weights.append(1)
        else:
            if (
                ...
                signals.append(
                    (
                        "MACD死叉",
                        -1.5,
                        f"DIF({macd_val:.3f})刚刚下穿DEA({signal_val:.3f})，死叉确立，卖出信号强烈",
                    )
                )
                signal_weights.append(-1.5)
            else:
                signals.append(
                    (
                        "MACD死叉延续",
                        -1,
                        f"DIF({macd_val:.3f})<DEA({signal_val:.3f})，死叉延续，动能{'增强' if hist_val < hist_prev else '减弱'}",
                    )
                )
                signal_weights.append(-1)
        ...
        if current_price > latest["BOLL_UP"]:
            signals.append(
                (
                    "突破布林上轨",
                    -1,
                    f"股价({current_price:.2f})突破上轨({latest['BOLL_UP']:.2f})，超买严重，可能回调，带宽{boll_width:.1f}%",
                )
            )
            signal_weights.append(-1)
            ...
            signals.append(
                (
                    "跌破布林下轨",
                    1,
                    f"股价({current_price:.2f})跌破下轨({latest['BOLL_DOWN']:.2f})，超卖严重，可能反弹，带宽{boll_width:.1f}%",
                )
            )
            signal_weights.append(1)
        elif current_price > latest["BOLL_MID"]:
            distance = (
                (current_price - latest["BOLL_MID"])
                / (latest["BOLL_UP"] - latest["BOLL_MID"])
                * 100
            )
            signals.append(
                (
                    "布林中上",
                    0.3,
                    f"股价在中轨上方，偏强势，距离上轨{distance:.1f}%，带宽{boll_width:.1f}%",
                )
            )
            signal_weights.append(0.3)
        else:
            distance = (
                (latest["BOLL_MID"] - current_price)
                / (latest["BOLL_MID"] - latest["BOLL_DOWN"])
                * 100
            )
            signals.append(
                (
                    "布林中下",
                    -0.3,
                    f"股价在中轨下方，偏弱势，距离下轨{distance:.1f}%，带宽{boll_width:.1f}%",
                )
            )
            signal_weights.append(-0.3)
        ...
        if last_pattern["signal"] in ["强烈看涨"]:
            signals.append(
                (
                    ...
                    1.5,
                    f"{last_pattern['explanation']}，可靠性{last_pattern['reliability']}",
                )
            )
            signal_weights.append(1.5)
            ...
            signals.append(
                (
                    last_pattern["pattern"],
                    0.5,
                    f"{last_pattern['explanation']}，需结合趋势判断，可靠性{last_pattern['reliability']}",
                )
            )
            signal_weights.append(0.5)
    ...
    if volume_ratio > 1.5:
        signals.append(
            (
                "放量",
                0.5 if change_pct > 0 else -0.5,
                f"近5日均量/20日均量={volume_ratio:.2f}，{'放量上涨，资金流入' if change_pct > 0 else '放量下跌，资金出逃'}",
            )
        )
        ...
        signals.append(
            (
                "缩量",
                0.3 if change_pct < 0 else -0.3,
                f"近5日均量/20日均量={volume_ratio:.2f}，{'缩量下跌，抛压减轻' if change_pct < 0 else '缩量上涨，上涨乏力'}",
            )
        )
...
├────────┼──────────┼──────────┼──────────┼─────────────────────────────────────┤"""
    ...
    for ma in ["MA5", "MA10", "MA20", "MA60"]:
        if ma in df.columns and not pd.isna(latest[ma]):
            ...
            if ma == "MA5":
                desc = "短期趋势" + (
                    "向上" if ma_change > 0 else "向下" if ma_change < 0 else "走平"
                )
            elif ma == "MA10":
                desc = "中短期趋势" + (
                    "向好" if ma_change > 0 else "走弱" if ma_change < 0 else "震荡"
                )
            elif ma == "MA20":
                desc = "中期趋势" + (
                    "向上" if ma_change > 0 else "向下" if ma_change < 0 else "盘整"
                )
            else:
                desc = "长期趋势" + (
                    "向好" if ma_change > 0 else "走弱" if ma_change < 0 else "平稳"
                )
...
"""
            ...
            distance_to_down = (
                (current_price - boll_down) / (boll_mid - boll_down) * 100
            )
            report += f"""
...
def main():
    parser = argparse.ArgumentParser(
        description="A股K线分析工具 v1.0.4 - 超详细结构化报告"
    )
    parser.add_argument("--code", required=True, help="股票代码 (如: 600409)")
    parser.add_argument(
        "--period", default="daily", choices=["daily", "weekly", "monthly"]
    )
    ...
    if args.realtime or args.report:
        ...
        if realtime_data:
            print(
                f"✓ 实时价格: {realtime_data['price']:.2f} ({realtime_data['change_pct']:+.2f}%)"
            )
        else:
            print("✗ 获取实时价格失败")

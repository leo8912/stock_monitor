...
class Updater:
    ...
    def __init__(self, update_package: str, target_dir: str, main_exe: str, pid: Optional[int] = None):
    ...
    def log(self, message: str, progress: int = 0, detail: str = "", level: str = "INFO"):
        """记录日志并更新GUI"""
        # 记录到日志文件
    ...
    def backup_current_version(self, backup_dir: Path) -> bool:
        ...
        try:
            ...
            for item in self.target_dir.iterdir():
                if item.name.startswith('.') or item.name in ['logs', 'cache', '.stock_monitor']:
                    continue
    ...
    def start_main_program(self) -> bool:
        ...
        try:
            ...
            # 使用Popen分离进程
            subprocess.Popen([str(main_exe_path)], 
    ...
    def self_delete(self):
        ...
        try:
            # 创建批处理脚本来删除自己
            updater_path = Path(sys.executable if getattr(sys, 'frozen', False) else __file__)

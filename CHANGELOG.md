...
### 🐛 修复
- **增强网络连接稳定性**: 修复系统启动时因网络未就绪导致的数据获取失败问题
- **改进数据处理容错性**: 增强对None值的处理，避免float()函数处理NoneType错误
- **优化港股数据获取**: 解决zhconv资源文件缺失导致的港股数据获取失败问题
- **完善错误回退机制**: 当网络请求失败时自动使用本地缓存数据，提高应用稳定性
...
### 🐛 修复
- **增强开机后行情获取稳定性**: 增加详细的日志记录和重试机制，帮助诊断开机后行情获取失败问题
- **优化数据获取重试机制**: 在后台刷新和预加载过程中增加重试机制，提高数据获取成功率
...
### 🐛 修复
- **修复开机后初次获取行情失败问题**: 增加启动延迟和数据获取重试机制，确保系统启动后能正确获取股票行情数据
...
### 🐛 修复
- **修复编译版本港股搜索失效问题**: 修复PyInstaller打包后资源文件路径处理问题，确保在线升级后港股数据能被正确加载
...
### 🎨 优化
- **优化港股搜索功能**: 改进港股搜索算法，支持更灵活的搜索方式
- **重构搜索匹配逻辑**: 优化股票搜索的匹配评分机制，提高搜索准确性

### 🐛 修复
- **修复港股无法添加问题**: 完善港股代码处理逻辑，确保港股能被正确添加和保存
- **修复设置界面初始化错误**: 调整设置界面组件初始化顺序，解决右键点击设置时报错的问题
- **修复模块依赖问题**: 添加缺失的 `openpyxl`、`pandas` 和 `zhconv` 依赖
- **修复港股搜索问题**: 优化港股搜索匹配算法，确保能正确搜索到港股
...
### 🎨 优化
- **移除延迟加载机制**: 为提高代码简洁性，移除所有延迟加载和节流机制，使操作更加直接
...
### 🎨 优化
- **优化配置保存机制**: 实现配置保存节流机制，避免频繁保存导致的界面卡顿问题
- **提升添加股票性能**: 优化股票搜索和添加功能，使用延迟执行避免界面卡顿

### 🐛 修复
- **修复配置丢失问题**: 改进配置保存逻辑，确保在保存时不会覆盖其他配置项
...
### ✨ 新功能
- **新增港股支持**: 实现从香港交易所官网自动获取港股列表数据
  - 使用 `requests` 库下载港股Excel文件
  - 使用 `pandas` 解析Excel文件内容（修复了之前使用openpyxl无法正确读取的问题）
  - 自动整合港股数据到股票数据库中
- **完善证券类型支持**: 增强系统对多种证券类型的支持
  - 添加港股标识 🇭🇰 用于区分显示
  - 确保指数、A股、港股具有统一的处理逻辑

### 🐛 修复
- **检查更新功能失效问题**: 修复设置面板中检查更新按钮点击无反应的问题
  - 添加备用更新地址，使用 https://ghfast.top/ 提供的GitHub镜像加速服务
  - 优化更新检查逻辑，当主地址无法访问时自动切换到备用地址
- **港股数据获取问题**: 修复港股数据获取不完整的问题
  - 修复Excel文件解析逻辑，正确读取香港交易所提供的证券列表
  - 改进数据提取算法，确保获取完整的港股数据
  - 添加繁体中文到简体中文的自动转换功能

### 🔧 技术改进
- **数据获取模块优化**: 增强 [updater.py](file:///d%3A/code/stock/stock_monitor/data/updater.py) 模块功能
  - 新增 `fetch_hk_stocks` 函数专门处理港股数据获取
  - 改进 `fetch_all_stocks` 函数整合港股、A股和指数数据
  - 优化错误处理和日志记录机制
  - 使用 pandas 替代 openpyxl 以更好地处理Excel文件格式
  - 集成 zhconv 库实现港股名称的繁简转换
...
### 🐛 修复
- **修复代码合并冲突**: 清理main.py中的Git合并冲突标记，解决语法错误导致程序无法启动的问题
- **修复缺失的工具函数**: 在utils/helpers.py中添加handle_exception函数，解决导入错误问题
...
- **完善API文档和代码注释**: 为关键模块添加详细文档字符串，完善API.md使用示例和最佳实践
- **统一错误处理模式**: 创建统一的异常处理工具函数，简化项目中的异常处理代码
- **消除重复代码**: 统一股票代码格式化逻辑，避免在多个文件中重复实现

...
### 7. 自动更新功能
- 采用 **BAT脚本无感热更新** 方案，彻底解决文件占用问题
- 支持自动检测并下载新版本
- 通过GitHub API检测更新,支持备用加速下载源(https://ghfast.top/)
- 下载更新包 -> 生成BAT脚本 -> 主程序退出 -> 脚本替换文件 -> 自动重启
- 更新过程中显示绿色CMD进度窗口，清晰直观
- 内置 SHA256 哈希校验，防止文件损坏或篡改
...
本软件采用 **BAT脚本更新方案** 实现自动更新，相比旧版 `updater.exe` 更加轻量且稳健。

**更新流程**:
1.  **下载**: 主程序下载 ZIP 更新包，并校验 SHA256 哈希值。
2.  **准备**: 将更新包解压到临时目录，并生成 `update.bat` 脚本。
3.  **交接**: 主程序启动 BAT 脚本后 **立即退出** (`os._exit(0)`)，彻底释放所有文件锁。
4.  **替换**: BAT 脚本等待主程序 PID 消失后，执行 `xcopy` 替换所有文件。
5.  **重启**: 更新完成后，脚本重新启动主程序并自删除。

**优势**:
- ✅ **零外部依赖**: 不需要额外的 `updater.exe`，减小体积。
- ✅ **解决占用的终极方案**: 主程序完全退出，不存在 "文件被占用" 的可能性。
- ✅ **兼容性强**: 使用 `GBK` 编码的 BAT 脚本，完美兼容中文 Windows 环境。
- ✅ **可视化**: 更新时弹出绿色 CMD 窗口，进度一目了然。
